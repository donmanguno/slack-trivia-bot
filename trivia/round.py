...
class RoundManager:
    ...
    async def _handle_correct_answer(
        ...
        await self._post_msg(
            round_.channel_id,
            f":white_check_mark: <@{user_id}> got it!{f' ({result.score:.1f}% match)' if result.score < 100 else ''} +{total_points} point{'s' if total_points != 1 else ''}{bonus_text}"
            f"\nThe answer is *{question.correct_answer.strip()}*. "
            f"{f'\nAlternate answers: {', '.join(question.alternate_answers)} ' if question.alternate_answers else ''}"
        )
